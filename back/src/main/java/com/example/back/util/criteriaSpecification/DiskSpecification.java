    ...
    public static Specification<Disk> byCriteria(DiskSearchCriteria criteria){
        ...
        return ((root, query, criteriaBuilder) -> {
            ...
            if (!criteria.getDevice().isBlank()){
                predicates.add(criteriaBuilder.equal(metricJoin.get("device"), criteria.getDevice()));
            }
            if (!criteria.getMountpoint().isBlank()){
                predicates.add(criteriaBuilder.equal(metricJoin.get("mountpoint"), criteria.getMountpoint()));
            }
